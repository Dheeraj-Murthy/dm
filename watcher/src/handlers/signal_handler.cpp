...
void SignalHandler::handle(const std::string& channel, const json& data,
                           const RuleLoader& rules, Connection& conn) {
    (void)channel;
        ...
        if (rule.enabled) {
            ...
            for (const auto& action : rule.actions) {
                if (action.set_phase_timing) {
                    ...
                    std::string sql = "UPDATE phases SET green_time=" +
                                    std::to_string(action.set_phase_timing->green_time) +
                ...
                if (action.set_active_phase) {
                    ...
                    std::string sql = "UPDATE approaches SET green_time=" +
                                    std::to_string(action.set_active_phase->green_time) +
                ...
                if (action.enable_corridor_free) {
                    ...
                    std::string sql = "UPDATE corridors SET green_time=" +
                                    std::to_string(action.enable_corridor_free->green_time) +
                ...
                if (action.set_mode) {
                    ...
                    std::string mode_str = action.set_mode->mode == SignalMode::DYNAMIC ? "DYNAMIC" :
                                            ...
                                            action.set_mode->mode == SignalMode::MANUAL    ? "MANUAL" : "ECA_CONTROLLED";
